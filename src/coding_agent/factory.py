...
def _load_global_memory(workspace: Path) -> str:
    primary = workspace / ".codeclaw" / "MEMORY.md"
    legacy = workspace / ".xingclaw" / "MEMORY.md"
    path = legacy if legacy.exists() and not primary.exists() else primary
    if not path.exists():
        return ""
    try:
...
def create_agent_session(options: AgentSessionOptions | CreateAgentSessionOptions) -> AgentSession:
    ...
    if prompt_cache is None and resources and resources.settings.prompt_cache:
        prompt_cache = resources.settings.prompt_cache  # type: ignore[assignment]
    if prompt_cache is None:
        prompt_cache = "off"

    prompt_cache_ttl = options.prompt_cache_ttl
    if prompt_cache_ttl is None and resources and resources.settings.prompt_cache_ttl:
        prompt_cache_ttl = resources.settings.prompt_cache_ttl  # type: ignore[assignment]
    ...
    if mcp_client is None:
        remote_mcp_servers = parse_remote_mcp_server_configs(mcp_servers)
        if remote_mcp_servers:
            mcp_client = RemoteMCPClient(remote_mcp_servers)
    ...
    concrete = AgentSessionOptions(
        ...
        llm_memory_reflection=options.llm_memory_reflection,
        max_memory_reflection_items=options.max_memory_reflection_items,
        memory_prompt_limit=options.memory_prompt_limit,
        memory_injection_char_budget=options.memory_injection_char_budget,
        ...
        skills=[*options.skills, *loaded_skills.skills],
        max_routed_skills=options.max_routed_skills,
        skill_injection_char_budget=options.skill_injection_char_budget,
        skill_embedding_recall=options.skill_embedding_recall,
        skill_embedding_backend=options.skill_embedding_backend,
        skill_embedding_model_path=options.skill_embedding_model_path,
