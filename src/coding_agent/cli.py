...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CodeClaw coding-agent CLI")
    ...
    parser.add_argument(
        ...
        default=None,
        help="Enable provider prompt caching when supported",
    )
    parser.add_argument(
        "--prompt-cache-ttl",
        choices=["5m", "1h"],
        default=None,
        help="Prompt cache TTL when supported by the provider",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable reading .codeclaw/{settings,prompt,tools}",
    )
    return parser
