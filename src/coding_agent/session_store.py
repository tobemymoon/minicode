...
RUNTIME_DIR = ".codeclaw"
LEGACY_RUNTIME_DIR = ".xingclaw"
...
def new_session_id() -> str:
    created_at = datetime.now().astimezone().strftime("%Y%m%d_%H%M%S_%f")
...
class SessionStore:
    def __init__(self, workspace_dir: str | Path, session_id: str) -> None:
        ...
        primary_root = self.workspace_dir / RUNTIME_DIR / "sessions" / session_id
        legacy_root = self.workspace_dir / LEGACY_RUNTIME_DIR / "sessions" / session_id
        self.root = legacy_root if legacy_root.exists() and not primary_root.exists() else primary_root
        ...
        self.summary_file = self.root / "session_summary.jsonl"
    ...
    def append_session_summary(self, summary: dict[str, Any]) -> None:
        ...
        with self.summary_file.open("a", encoding="utf-8") as fp:
            fp.write(json.dumps(entry, ensure_ascii=False, default=str) + "\n")
        self.touch_updated_at()

    def load_session_summaries(self) -> list[dict[str, Any]]:
        if not self.summary_file.exists():
            return []
        out: list[dict[str, Any]] = []
        for line in self.summary_file.read_text(encoding="utf-8").splitlines():
            line = line.strip()
            if not line:
                continue
