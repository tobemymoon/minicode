...
def _create_fresh_session(old: AgentSession) -> AgentSession:
    ...
    return AgentSession(
        AgentSessionOptions(
            ...
            storage_backend=old.storage_backend,
            ...
            prompt_cache=old.prompt_cache,
            prompt_cache_ttl=old.prompt_cache_ttl,
...
def _format_usage(session: AgentSession) -> str:
    ...
    lines = [
        f"prompt_cache={session.prompt_cache}",
        f"prompt_cache_ttl={session.prompt_cache_ttl or 'default'}",
        "last:",
        f"  input_tokens={last.get('input_tokens', 0)}",
        f"  output_tokens={last.get('output_tokens', 0)}",
        f"  cache_read={last.get('cache_read', 0)}",
        f"  cache_write={last.get('cache_write', 0)}",
        f"  total_tokens={last.get('total_tokens', 0)}",
        f"  cost_total={float(cost.get('total', 0.0)):.6f}",
        "cumulative:",
        f"  input_tokens={cumulative.get('input_tokens', 0)}",
        f"  output_tokens={cumulative.get('output_tokens', 0)}",
        f"  cache_read={cumulative.get('cache_read', 0)}",
        f"  cache_write={cumulative.get('cache_write', 0)}",
        f"  total_tokens={cumulative.get('total_tokens', 0)}",
        f"  total_cost={float(cumulative.get('total_cost', 0.0)):.6f}",
    ]
    return "\n".join(lines)
