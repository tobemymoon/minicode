...
READ_ONLY_TOOL_NAMES = {"read", "read_file", "grep", "find", "ls", "list_dir", "read_artifact", "search_artifact"}
...
def create_builtin_tools(
    ...
    *,
    ...
    async def read_artifact_tool(tool_call_id: str, params: dict[str, Any], signal=None, on_update=None) -> AgentToolResult:
        ...
        max_chars = min(int(params.get("max_chars", 1200)), 2000)
        if not artifact_id:
            return AgentToolResult(content=[TextContent(text="Missing artifact_id")], details={})
        try:
            text, details = artifact_store.read_text(artifact_id, offset=offset, max_chars=max_chars)
        except Exception as exc:
            return AgentToolResult(content=[TextContent(text=f"Failed to read artifact: {exc}")], details={"error": str(exc)})
        return AgentToolResult(content=[TextContent(text=text if text else "(empty)")], details=details)

    async def search_artifact_tool(
        tool_call_id: str,
        params: dict[str, Any],
        signal=None,
        on_update=None,
        ...
        if not artifact_id:
            return AgentToolResult(content=[TextContent(text="Missing artifact_id")], details={})
        if not query:
            return AgentToolResult(content=[TextContent(text="Missing query")], details={})
        try:
            results, details = artifact_store.search_text(artifact_id, query, max_results=max_results)
        except Exception as exc:
            return AgentToolResult(
                content=[TextContent(text=f"Failed to search artifact: {exc}")],
                details={"error": str(exc)},
            )
        if not results:
            return AgentToolResult(content=[TextContent(text="No matches")], details=details)
        lines = [
            f"line={item['line']} offset={item['offset']} snippet={item['snippet']}"
            for item in results
        ]
        return AgentToolResult(content=[TextContent(text="\n".join(lines))], details={**details, "results": results})
    ...
    async def bash_tool(tool_call_id: str, params: dict[str, Any], signal=None, on_update=None) -> AgentToolResult:
        ...
        try:
            ...
            out_text = completed.stdout.decode("utf-8", errors="replace")
            err_text = completed.stderr.decode("utf-8", errors="replace")
            merged = f"$ {command}\n{out_text}"
            if err_text:
                merged += ("\n[stderr]\n" + err_text)
            return AgentToolResult(
            ...
            return AgentToolResult(content=[TextContent(text=f"Invalid command: {exc}")], details={"error": str(exc)})
        except OSError as exc:
            return AgentToolResult(content=[TextContent(text=f"Command failed: {exc}")], details={"error": str(exc)})
    ...
    if _allow("read_artifact"):
        tools.append(
            AgentTool(
                name="read_artifact",
                label="Read Externalized Artifact",
                description="读取被上下文压缩外置化的长工具结果。仅在用户问题必须依赖原文细节时使用，优先读取小片段。",
                parameters={
                    "type": "object",
                    "properties": {
                        "artifact_id": {"type": "string", "description": "artifact id，例如 art_0123abcd..."},
                        "offset": {"type": "number", "description": "从第几个字符开始读取，默认 0"},
                        "max_chars": {"type": "number", "description": "最多返回字符数，默认 1200，硬限制 2000"},
                    },
                    "required": ["artifact_id"],
                    "additionalProperties": False,
                },
                execute=read_artifact_tool,
            )
        )

    if _allow("search_artifact"):
        tools.append(
            AgentTool(
                name="search_artifact",
                label="Search Externalized Artifact",
                description="在外置 artifact 中按关键词检索相关行，返回 line、offset 和短 snippet。读取原文前应先用它定位。",
                parameters={
                    "type": "object",
                    "properties": {
                        "artifact_id": {"type": "string", "description": "artifact id，例如 art_0123abcd..."},
                        "query": {"type": "string", "description": "要检索的关键词或短语"},
                        "max_results": {"type": "number", "description": "最多返回匹配数，默认 5，硬限制 20"},
                    },
                    "required": ["artifact_id", "query"],
                    "additionalProperties": False,
                },
                execute=search_artifact_tool,
            )
        )
