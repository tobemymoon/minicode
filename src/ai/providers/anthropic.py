...
def _apply_usage(out: Any, usage: dict[str, Any]) -> None:
...
def _cache_control(ttl: str | None) -> dict[str, Any]:
    value: dict[str, Any] = {"type": "ephemeral"}
    if ttl:
        value["ttl"] = ttl
    return value


def _apply_explicit_prompt_cache(
...
def stream_anthropic(
    ...
    async def _run() -> None:
        ...
        try:
            ...
            if not api_key and "moonshot.cn" in model.base_url:
                api_key = os.getenv("MOONSHOT_API_KEY") or os.getenv("KIMI_API_KEY")
            if not api_key:
                if "moonshot.cn" in model.base_url:
                    raise RuntimeError("Missing API key: set ANTHROPIC_API_KEY or MOONSHOT_API_KEY")
            ...
            if resolved_options.prompt_cache == "auto":
                payload["cache_control"] = _cache_control(resolved_options.prompt_cache_ttl)
            if context.system_prompt and resolved_options.prompt_cache != "explicit":
            ...
            if resolved_options.prompt_cache == "explicit":
                _apply_explicit_prompt_cache(
                    payload,
                    system_prompt=context.system_prompt,
                    tools=tools,
                    ttl=resolved_options.prompt_cache_ttl,
                )
            elif tools:
                payload["tools"] = tools
